...
namespace ReactiveUI.Myra
{
	public class ActivationForViewFetcher : IActivationForViewFetcher
	{
		...
		public IObservable<bool> GetActivationForView(IActivatableView? view)
		{
			...
			{
				var placeChanged = Observable.FromEvent<MyraEventHandler, bool>(
					eventHandler =>
					{
				...
				{
					var windowClosed = Observable.FromEvent<MyraEventHandler, bool>(
						eventHandler =>
						{
							void Handler(object? sender, MyraEventArgs e) => eventHandler(window.Visible && window.IsPlaced);
							return Handler;
						},
						h => window.Closed += h,
						h => window.Closed -= h);
